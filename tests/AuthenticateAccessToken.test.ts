...
describe("AuthenticateAccessToken", () => {
  it("returns a non-expired cached principal without verifying the token again", async () => {
    ...
    const useCase = new AuthenticateAccessToken(verifier, cache, { cacheTtlSeconds: 300, now: () => 1_000 });
    await expect(useCase.execute("token", ["receipts:scan"])).resolves.toEqual(principal);
    expect(verifier.verify).not.toHaveBeenCalled();
  });

  it("verifies a cache miss and caps cache TTL at token expiry", async () => {
    const cache = cacheWith(null);
    const verifier: AccessTokenVerifier = { verify: jest.fn(async () => principal) };
    const useCase = new AuthenticateAccessToken(verifier, cache, { cacheTtlSeconds: 600, now: () => 1_000 });
    await useCase.execute("token", ["receipts:scan"]);
    expect(verifier.verify).toHaveBeenCalledWith("token");
    expect(cache.set).toHaveBeenCalledWith(expect.stringMatching(/^[a-f0-9]{64}$/), principal, 300);
  });
  ...
  it("rejects a principal that lacks a required scope", async () => {
    ...
    await expect(useCase.execute("token", ["receipts:scan"])).rejects.toThrow(ForbiddenError);
  });
  ...
  it("bypasses Redis failure and verifies directly", async () => {
    const cache: AuthenticationCache = {
      ...
      set: jest.fn(async () => { throw new Error("redis unavailable"); }),
      delete: jest.fn(async () => undefined),
    };
    const verifier: AccessTokenVerifier = { verify: jest.fn(async () => principal) };
    const useCase = new AuthenticateAccessToken(verifier, cache, { cacheTtlSeconds: 300, now: () => 1_000 });
