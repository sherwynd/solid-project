...
import {
  ...
  type RedisClient,
} from "../src/infrastructure/redis/RedisAuthenticationCache.js";
...
describe("RedisAuthenticationCache", () => {
  ...
  it("deletes malformed cached authentication data", async () => {
    const redis = new FakeRedis();
    redis.values.set(
      "receipt-api:auth:abc123",
      JSON.stringify({ subject: "missing-fields" }),
    );
    const cache = new RedisAuthenticationCache(redis);
  ...
  it("deletes invalid cached JSON", async () => {
    const redis = new FakeRedis();
    redis.values.set("receipt-api:auth:abc123", "{not-json");
    const cache = new RedisAuthenticationCache(redis);
    await expect(cache.get("abc123")).resolves.toBeNull();
    expect(redis.values.has("receipt-api:auth:abc123")).toBe(false);
  });
