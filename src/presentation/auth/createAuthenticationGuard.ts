...
import {
  ForbiddenError,
  UnauthorizedError,
} from "../../application/errors/AuthenticationErrors.js";
...
export function createAuthenticationGuard(
  ...
  return async (
    ...
    try {
      ...
      request.authPrincipal = await authenticator.execute(
        token,
        requiredScopes,
      );
    } catch (error) {
      if (error instanceof ForbiddenError) {
        reply.header(
          "WWW-Authenticate",
          `Bearer realm="receipt-api", error="insufficient_scope", scope="${requiredScopes.join(" ")}"`,
        );
