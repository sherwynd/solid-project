...
export class RedisAuthenticationCache implements AuthenticationCache {
  ...
  static create(redisUrl: string, keyPrefix?: string): RedisAuthenticationCache {
  ...
  async get(tokenHash: string): Promise<AuthPrincipal | null> {
    ...
    if (!raw) return null;
    const parsed = authPrincipalSchema.safeParse(JSON.parse(raw) as unknown);
    if (!parsed.success) {
  ...
  async set(tokenHash: string, principal: AuthPrincipal, ttlSeconds: number): Promise<void> {
    await this.client.set(this.key(tokenHash), JSON.stringify(principal), "EX", ttlSeconds);
  }
